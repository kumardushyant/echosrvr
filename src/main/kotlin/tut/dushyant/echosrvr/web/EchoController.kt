  ...
  fun get(request: HttpServletRequest): Map<String, Map<String, String>> {
    ...
    val params =
        request.parameterMap
            .map { (paramName, paramValues) -> paramName to paramValues.joinToString(",") }
            .toMap()
    val attributes =
        request.attributeNames
            .toList()
            .map { attrName -> attrName to request.getAttribute(attrName).toString() }
            .toMap()
            .filter { (key, _) -> !key.startsWith("org.springframework") }
